...
```kotlin
dependencies {
    implementation("io.github.btown021:mybatis-plus-kt:1.0.0")
}
```
...
<dependency>
    ...
    <artifactId>mybatis-plus-kt</artifactId>
...
**必须**通过此方法创建 `QueryWrapper`，不能直接 `QueryWrapper<T>()`—— 后者未设置 `entityClass`，列名解析会失败。
部分 Service 多层继承场景下未设置 `entityClass`会导致异常，建议始终使用此方法保证`entityClass`正确设置。

```kotlin
val wrapper = userService.createQueryWrapper()
```
...
| `in` | `User::role in listOf("A","B")` | `IN ('A','B')` | 空集合→跳过 |
