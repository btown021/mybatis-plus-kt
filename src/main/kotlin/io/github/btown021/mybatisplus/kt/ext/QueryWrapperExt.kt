 ...
 * @return 分页结果
 */
fun <T, E : IPage<T>> QueryWrapper<T>.selectPage(page: E): IPage<T> = resolveServiceBean(this).page(page, this)

/**
 * 列表查询
 * @return 查询结果列表，无匹配时返回空列表
 */
fun <T> QueryWrapper<T>.selectList(): List<T> = resolveServiceBean(this).list(this)

/**
 * 单条查询
 * @return 匹配的实体，无匹配返回 null；多条匹配时可能抛出 TooManyResultsException
 */
fun <T> QueryWrapper<T>.selectOne(): T? = resolveServiceBean(this).getOne(this)

/**
 * 计数查询
 * @return 匹配行数
 */
fun <T> QueryWrapper<T>.selectCount(): Long = resolveServiceBean(this).count(this)

/**
 * Map 列表查询 — 每行以 Map<String, Any> 返回，key 为数据库列名
 */
fun <T> QueryWrapper<T>.selectMapList(): List<Map<String, Any>> = resolveServiceBean(this).listMaps(this)

/**
 * 单条 Map 查询
 * @return 单行 Map，无匹配返回 null
 */
fun <T> QueryWrapper<T>.selectMap(): Map<String, Any> = resolveServiceBean(this).getMap(this)
 ...
 * @return true 表示操作成功
 */
fun <T> QueryWrapper<T>.remove(): Boolean = resolveServiceBean(this).remove(this)
